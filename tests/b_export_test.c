...
static t_tests		*config(char *file_name, char **cmds);
static void			assert_env(t_tests *vars, char **expected);

MU_TEST(test_passing_only_export_should_be_export_env) {
	// CONFIG
	char			*file1 = "./files/export_1.txt";
	t_tests			*vars = config(file1, ft_split("export ", 32));
...
MU_TEST(passing_export_TEST_without_value_should_only_add_in_export_env) {
	// CONFIG
	t_tests			*vars = config("./files/export_2.txt", ft_split("export\aTEST", 7));
...
MU_TEST(passing_export_TEST_with_a_empty_value_should_add_in_export_env_and_env) {
	// CONFIG
	t_tests			*vars = config("./files/export_3.txt", ft_split("export\aTEST=", 7));
	vars->act 		= vars->mini->tokens->token;
	vars->show		= ft_split("export\a", 7);
	char			*expected_env[] 	= { "USER=dapaulin",
											"TERM=xterm-256color",
...
MU_TEST(passing_export_TEST_with_90_in_value_should_add_in_export_env_and_env) {
	// CONFIG
	t_tests			*vars = config("./files/export_4.txt", ft_split("export\aTEST=98", 7));
	vars->act 		= vars->mini->tokens->token;
	vars->show		= ft_split("export\a", 7);
	char			*expected_env[] 	= { "USER=dapaulin",
											"TERM=xterm-256color",
...
MU_TEST(passing_export_TEST_without_value_with_an_existing_variable_TEST_with_90_in_value_must_not_change) {
	// CONFIG
	t_tests			*vars = config("./files/export_5.txt", ft_split("export\aTEST", 7));
	vars->act 		= vars->mini->tokens->token;
...
MU_TEST(test) {
	// CONFIG
	t_tests			*vars = config("./files/export_6.txt", ft_split("export\aTEST\aMARIO=curry\aLANG=BR\aVIRUS=", 7));
	vars->act 		= vars->mini->tokens->token;
	vars->show		= ft_split("export\a", 7);
	char			*expected_env[] 	= { "USER=dapaulin",
											"TERM=xterm-256color",
...
static t_tests *config(char *file_name, char **cmds)
{
	...
	vars->file_name = ft_strdup(file_name);
	vars->fd = 0;
	vars->bkp = dup(1);
